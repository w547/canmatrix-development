...
def test_initial_value_zero_raw_nonzero():
    dbc = io.BytesIO(textwrap.dedent(u'''\
            BO_ 287 ABM_State: 8 Tester
              SG_ ABM_IMU_VertA : 31|14@0+ (0.01,-49.05) [-49.05|114.78] "m/s2" Tester

            BA_DEF_ SG_ "GenSigStartValue" FLOAT 0 100000000000;
    ''').encode('utf-8'))

    matrix = canmatrix.formats.dbc.load(dbc, dbcImportEncoding="utf8")
    signal = matrix.frames[0].signals[0]
    assert float(signal.initial_value) == pytest.approx(0.0, abs=1e-10)
    assert signal.phys2raw(None) == 4905

    outdbc = io.BytesIO()
    canmatrix.formats.dump(matrix, outdbc, "dbc")
    outdbc.seek(0)
    content = outdbc.read().decode('utf-8')

    outdbc.seek(0)
    matrix2 = canmatrix.formats.dbc.load(outdbc, dbcImportEncoding="utf8")
    signal2 = matrix2.frames[0].signals[0]
    assert float(signal2.initial_value) == pytest.approx(0.0, abs=1e-10)
    assert signal2.phys2raw(None) == 4905
