...
def api_convert():
    ...
    # DBC 编码选项 (自动检测编码以支持中文GBK/Big5/UTF-8)
    dbc_import_encoding = request.form.get('dbc_import_encoding', 'auto')
    dbc_export_encoding = request.form.get('dbc_export_encoding', 'utf-8')
    if not dbc_import_encoding:
        dbc_import_encoding = 'utf-8'
    if not dbc_export_encoding:
        dbc_export_encoding = 'utf-8'

    # 判断是否为文本类格式（编码检测仅对文本格式有意义）
    ...
    # 注释编码：跟随导入/导出编码（如果用户设置了"auto"则自动解析）
